...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `CCv6`.`all_users`
(
    `id` INT
);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `CCv6`.`all_venues`
(
    `id` INT
);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `CCv6`.`all_carves`
(
    `id` INT
);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `CCv6`.`all_media`
(
    `media_id`    INT,
    `poster`      INT,
    `carve`       INT,
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `CCv6`.`all_messages`
(
    `id` INT
);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `CCv6`.`all_likes`
(
    `id` INT
);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `CCv6`.`all_carve_attendees`
(
    `id` INT
);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `CCv6`.`all_follows`
(
    `follow_id`  INT,
    `user_id1`   INT,
    `user_id2`   INT,
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `CCv6`.`all_comments`
(
    `id` INT
);
...
DELIMITER $$
USE `CCv6`$$
CREATE PROCEDURE `add_venue`(in vname VARCHAR(40), in cityNear VARCHAR(45), in st VARCHAR(2),
...
CREATE PROCEDURE `add_carve`(in carveName varchar(40), in creatorId int, in venueId int,
                             in carveType set ('open','buddy'), in athlete int, in photo int, in date varchar(30),
...
DELIMITER $$
USE `CCv6`$$
CREATE PROCEDURE `update_venues`(in id int, in vname VARCHAR(40), in cityNear VARCHAR(45), in st VARCHAR(2),
                                 in snowSports set ('snowboard','ski','snowmobile'),
                                 in waterSports set ('surf','waterSki'), in landSports set ('skateboard','BMX'),
...
DELIMITER $$
USE `CCv6`$$
CREATE PROCEDURE `update_venue`(in id int, in vname VARCHAR(40), in cityNear VARCHAR(45), in st VARCHAR(2),
                                in snowSports set ('snowboard','ski','snowmobile'),
                                in waterSports set ('surf','waterSki'), in landSports set ('skateboard','BMX'),
                                in airSports set ('skydive','hangGlide'), in des varchar(100))
BEGIN
    update venues
...
DELIMITER $$
USE `CCv6`$$
CREATE PROCEDURE `add_carve_attendee`(in car int, in us int,
                                      in userType SET ('photographer', 'filmographer', 'droneOperator', 'athlete', 'proAthlete', 'fan'))
BEGIN
...
DELIMITER $$
USE `CCv6`$$
CREATE PROCEDURE `update_carve_attendee`(in id int, in car int, in us int,
                                         in userType SET ('photographer', 'filmographer', 'droneOperator', 'athlete', 'proAthlete', 'fan'))
BEGIN
...
CREATE PROCEDURE `update_carve`(in id int, in carveName varchar(40), in creatorId int, in venueId int,
                                in carveType set ('open','buddy'), in athlete int, in photo int, in dat varchar(30),
                                in com tinyint,
                                in sportsAdd set ('snowboard','ski','snowmobile','surf','waterSki','skateboard','BMX','skydive','hangGlide'))
BEGIN
    update carves
...
DELIMITER $$
USE `CCv6`$$
CREATE PROCEDURE `update_follow`(in id int, in user1 int, in user2 int, in ven int,
                                 in ty SET ('buddy', 'follow', 'block'))
BEGIN
    update follows
    set user_id1 = user1,
        user_id2 = user2,
...
DELIMITER $$
USE `CCv6`$$
CREATE PROCEDURE `update_medi`(in id int, in pos int, in ur varchar(50), in des varchar(100), in car int, in ven int,
                               in pro int)
BEGIN
    update media
    set poster      = pos,
...
DELIMITER $$
USE `CCv6`$$
CREATE PROCEDURE `update_like`(in id int, in pos int, in lord SET ('like', 'dislike'), in car int, in med int,
                               in com int)
BEGIN
    update likes
    set poster  = pos,
...
USE `CCv6`$$
CREATE
    DEFINER =`root`@`localhost` PROCEDURE `get_carve_likes`(in c_id int)

BEGIN

    select likes.*, carves.carve_id
    from likes
             join carves on carves.carve_id = likes.carve
    where carve_id = c_id;
...
USE `CCv6`$$
CREATE
    DEFINER =`root`@`localhost` PROCEDURE `update_message_read`(in id int)
BEGIN
    update messages
    set messages.read=1
    where message_id = id;
...
USE `CCv6`$$
CREATE
    DEFINER =`root`@`localhost` PROCEDURE `get_buddy_count`(in usr int)
BEGIN
...
USE `CCv6`$$
CREATE
    DEFINER =`root`@`localhost` PROCEDURE `count_venue_followers`(in ven int)
BEGIN
...
BEGIN
    ...
    from all_carves
             left join all_venues on all_carves.venue = all_venues.venue_id
             left join all_carve_attendees on all_carves.carve_id = all_carve_attendees.carve
             left join all_users on all_carve_attendees.user = all_users.user_id
...
USE `CCv6`$$
CREATE
    DEFINER =`root`@`localhost` PROCEDURE `get_media_likes`(in id int)

BEGIN

    select likes.media, media.media_id
    from likes
             join media
...
BEGIN
    ...
    select comments.media, media.media_id
    from comments
             join media
...
BEGIN
    ...
    select comments.comment, likes.comment
    from comments
             join likes on likes.comment = comments.comment_id;
...
SET SQL_MODE =
        'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';
CREATE USER 'nodeuser'@'localhost' IDENTIFIED WITH mysql_native_password BY 'Nodeuser@1234';
